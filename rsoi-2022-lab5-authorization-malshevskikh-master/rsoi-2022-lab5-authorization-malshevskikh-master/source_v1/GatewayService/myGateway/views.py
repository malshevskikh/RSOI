    ...
    if token is not None:
        ...
        if user is not None:
            if request.method == 'GET':
                ...
                flight_is_valid = requests.get("http://flight-service:8060/api/v1/flights/{}".format(flightNumber))
                ...
                payment_with_bonus = requests.patch("http://bonus-service:8050/api/v1/change_count", headers={"X-User-Name": user}, data=data_for_privilege)
    ...
    if token is not None:
        ...
        if user is not None:
            if request.method == 'GET':
                ...
                valid_ticket = requests.get("http://ticket-service:8070/api/v1/tickets/{}".format(ticketUid), headers={"X-User-Name": user})
                ...
                change_ticket = requests.patch("http://ticket-service:8070/api/v1/del_tick/{}".format(ticketUid), headers={"X-User-Name": user})
                ...
                return_money = requests.patch("http://bonus-service:8050/api/v1/return_money/{}".format(ticketUid), headers={"X-User-Name": user})
...
def gateway_get_all_flights(request):
    ...
    if token is not None:
        ...
        if (page is not None) and (size is not None):
            ...
            list_of_flight = requests.get("http://flight-service:8060/api/v1/flights?page={}&size={}".format(page, size))
            ...
            list_of_flight = requests.get("http://flight-service:8060/api/v1/flights")
            return JsonResponse(list_of_flight.json(), status=list_of_flight.status_code, safe=False)
    else:
        return JsonResponse({'message': 'не авторизован'}, status=status.HTTP_401_UNAUTHORIZED, safe=False)
